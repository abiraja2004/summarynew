...
import pickle
from nnlayers import load_data
...
if __name__ == "__main__":
    ...
    #conv_layer
    conv_layer = LenetConvPoolLayer(rng, project_layer.output,sentence_shape,filter_shape,pool_size,params=params[0:2])
    ...
    hidden_layer = FullConectedLayer(rng,hidden_input,hidden_input_shape[1],100,params=params[2:4])

    # regression_layer
    regession_layer = RegresstionLayer(rng,hidden_layer.output,100,1, params=params[4:6])
    ...
    pred = predict_function(X_valid[:100])
    print pred
    print Y_valid_rouge2[:100]
    print np.mean((pred - Y_valid_rouge2[:100]) ** 2)
